"""
Trains and saves g values as well as val acc for a kin model from kinxopt
"""

import torch, torchvision, time, inspect, pickle
from torchvision import transforms
import sys 
import kin_xopt as kinematics
...
if __name__ == '__main__':
    ...
    settings_dict = {
        ...
        "optimizers": [],
        #"optimizers": ['KinDub', 'Adam'], #, 'Adadelta', 'RMSprop'] 
        "kin_version": kin_version

    }

    # kin_version -> optimizers
    version_dict = {
        'Kb': 'KinDub',
        'Am': 'Adam',
        'Ad': 'ASGD'
    ...
    # select kin algo
    for v, vfull in version_dict.items():
        if v in kin_version:
            settings_dict['optimizers'].append(version_dict[v])

    assert(len(settings_dict['optimizers']) > 0)

    # adjust dimensions for linear model
    if model_arg == 'Linear':
    ...
    # save g vals
    for o in range(len(optimizer_list)):
        optObj = optimizer_list[o]
        print(type(optObj))
        if (isinstance(optObj, kinematics.KinDub)):
            optname = optimizer_names[o]
            print("name: {}".format(optimizer_names[o]))

...
def nesterov_update(state, w, dw, v, lr, weight_decay, momentum):
...
def LARS_update(state, w, dw, v, lr, weight_decay, momentum):
...
def opt_step(batch, state, update, param_schedule, step_number, weights, opt_state): # (2!) (update, param_schedule, ...) --> (state, update, ...)
    step_number += 1
    param_values = {k: f(step_number) for k, f in param_schedule.items()}
    # get grad norm (Kinematics)
    flat_grad = torch.Tensor().to('cuda')
    for w, v in zip(weights, opt_state):
        if w.requires_grad:
            flat_grad = torch.cat((flat_grad, w.grad.data.view(-1).to('cuda').type_as(flat_grad)))
    state['grad_norm'] = torch.norm(flat_grad, p = 2).item()
    # update parameters
    for w, v in zip(weights, opt_state):
        if w.requires_grad:
            update(state, w.data, w.grad.data, v, **param_values)
...
def opt_steps(batch, state):
    if not batch: return
    return {OPTS: [opt_step(batch, state, **opt) for opt in state[OPTS]]} # (!1) (**opt) --> (state, **opt) to enable loss to be passed
